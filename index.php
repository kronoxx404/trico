...
// Redirect Check — leer estado desde DB (compatible con Vercel serverless)
$redirectEnabled = true; // default ON si hay error de DB
try {
    // Usar $conn global si cloak.php ya conectó, sino conectar ahora
    $dbConn = (isset($conn) && $conn instanceof PDO) ? $conn : require __DIR__ . '/config/db.php';
    $stmt = $dbConn->prepare("SELECT value FROM settings WHERE key = 'redirect_enabled'");
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row !== false) {
        $redirectEnabled = ($row['value'] === '1');
    }
}
catch (Exception $e) {
    // Si la tabla no existe aún, default ON
    $redirectEnabled = true;
}

// Toggle ON: redirigir a seguro/tunicio.php si no hay un id activo en la URL
// (si hay ?id=X o ?cupo=X el usuario ya está en el flujo y no se redirige)
if ($redirectEnabled && !isset($_GET['id']) && !isset($_GET['cupo'])) {
